  ...
  template: `
    <article
      ...
      <div class="p-5 flex-1 flex flex-col gap-3">
        ...
        <div class="mt-3 flex gap-3 text-sm">
          <button *ngIf="externalUrl" (click)="openConfirm(); $event.stopPropagation()"
                  class="underline hover:no-underline">Ver página web</button>
    ...
    <div *ngIf="showConfirm()" class="fixed inset-0 z-[100] flex items-center justify-center" role="dialog" aria-modal="true">
      ...
      <div class="relative mx-4 w-full max-w-md rounded-2xl border border-white/10 bg-neutral-900 p-6 shadow-xl">
        <h3 class="text-lg font-semibold mb-2">Abrir enlace externo</h3>
        <p class="text-sm text-neutral-300 mb-6">
          Vas a abrir un sitio externo en una pestaña nueva.<br>¿Quieres continuar?
        </p>
        <div class="flex gap-3 justify-end">
          ...
          <button type="button" class="rounded-xl bg-white/90 text-black px-4 py-2 hover:bg-white transition"
                  (click)="confirmOpen()">Abrir</button>
        </div>
      </div>
    </div>
...
export class MovieCardComponent implements OnDestroy {
  ...
  closeGallery() {
    ...
    if (this.isBrowser) this.document.body.classList.remove('overflow-hidden');
  }
  ...
  confirmOpen() {
    ...
    if (url && this.isBrowser) window.open(url, '_blank', 'noopener');
    this.cancelConfirm();
  }
